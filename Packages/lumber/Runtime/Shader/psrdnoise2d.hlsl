...
#ifndef PSRDNOISELINCLUDE_INCLUDED
#define PSRDNOISELINCLUDE_INCLUDED
...
float2 modGLSL(float2 x, float2 y)
{
    return x - y * floor(x / y);
}

float3 modGLSL(float3 x, float3 y)
{
    return x - y * floor(x / y);
}

float4 modGLSL(float4 x, float4 y)
{
    return x - y * floor(x / y);
}
...
float3 psrdnoise2d_mod289(float3 x)
{
    return x - floor(x * (1.0 / 289.0)) * 289.0;
}

// Permutation polynomial (ring size 289 = 17*17)
float3 psrdnoise2d_permute(float3 x)
{
    return psrdnoise2d_mod289(((x * 34.0) + 10.0) * x);
}
...
// Map from a line to a diamond such that a shift maps to a rotation.
  float u = psrdnoise2d_permute(psrdnoise2d_permute(p.x) + p.y) * 0.0243902439 + rot; // Rotate by shift
  u = 4.0 * frac(u) - 2.0;
...
float3 psrdnoise(float2 pos, float2 per, float rot)
{
  ...
  // wrap points in (x,y), map to (u,v)
    float3 xw = modGLSL(float3(p0.x, p1.x, p2.x), per.x);
    float3 yw = modGLSL(float3(p0.y, p1.y, p2.y), per.y);
...
float3 srdnoise(float2 pos, float rot)
{
  ...
  // Avoid precision issues in permutation
    iuw = psrdnoise2d_mod289(iuw);
    ivw = psrdnoise2d_mod289(ivw);
...
void noise2d_float(float2 v, out float result) {
    result = snoise(v);
}

void noise2d_half(float2 v, out float result)
{
    result = snoise(v);
}

void psnoise_float(float2 v, float2 per, out float result)
{
    result = psnoise(v, per);
}

void psnoise_half(half2 v, half2 per, out half result)
{
    result = psnoise(v, per);
}

#endif // PSRDNOISELINCLUDE_INCLUDED